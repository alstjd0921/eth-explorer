...
router.get('/:pageId', async function (req, res, next) {
  ...
  await web3.eth.getTransaction(pageId, false, async function (err, tx) {
    ...
    await web3.eth.getTransactionReceipt(pageId, false, async function (
      err,
      txReceipt
    ) {
      let value = web3.utils.fromWei(tx.value, 'ether');
      let price = web3.utils.fromWei(tx.gasPrice, 'ether');
      let txFee = web3.utils.fromWei(
        (tx.gasPrice * txReceipt.gasUsed).toString(10)
      );
      let status;
      ...
      await web3.eth.getBlock(tx.blockNumber, false, function (err, block) {
        ...
        return res.render('tx', {
          ...
          to: tx.to,
          value: value,
          gasUsed: addComma(txReceipt.gasUsed),
          gasPrice: price,
          nonce: tx.nonce,
          timestamp: date,
          txFee: txFee,
